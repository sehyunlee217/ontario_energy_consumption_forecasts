...
fc2 |> autoplot(elec_demand) + 
    theme_ipsum() + labs(x = "Temperature (C)", y = "Monthly Demand (MW)",  title ="2 Year Forecasts for Energy Consumption in Ontario with ARIMA")
...
$$
Utilizing the past forecasts for temperatures, we can generate forecasts using this dynamic regression model as below. 

```{r}
fc1 <- forecast(fit_dy_reg, ontario_temps_fc_2years) 

fc1 |> autoplot(elec_demand) + 
    theme_ipsum() + labs(x = "Temperature (C)", y = "Monthly Demand (MW)", title ="2 Year Forecasts for Energy Consumption in Ontario with Dynamic Regression")
```

### Cross-Validation 

When we apply rolling-forecast-origin cross-validation to this time series, we find that the **dynamic regression model outperforms the AutoARIMA model in terms of error by 1-2%**. Although this error measurement captures only the training error, since it’s not appropriate to use a separate testing set in time series cross-validation, **Temperature proves to be a strong predictor for forecasting energy consumption in Ontario.**
